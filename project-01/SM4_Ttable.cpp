// sm4_ttable.cpp  —— 双 T‑table 优化版
#include <cstdint>
#include <array>
#include <iostream>
#include <iomanip>
#include <chrono>
#include <cstring>

using std::array; using std::uint32_t;

/* -------------------- S 盒 & 常量 -------------------- */
static const uint8_t SBOX[256] = {
  
};

static const uint32_t CK[32] = {
  0x00070E15,0x1C232A31,0x383F464D,0x545B6269,
  0x70777E85,0x8C939AA1,0xA8AFB6BD,0xC4CBD2D9,
  0xE0E7EEF5,0xFC030A11,0x181F262D,0x343B4249,
  0x50575E65,0x6C737A81,0x888F969D,0xA4ABB2B9,
  0xC0C7CED5,0xDCE3EAF1,0xF8FF060D,0x141B2229,
  0x30373E45,0x4C535A61,0x686F767D,0x848B9299,
  0xA0A7AEB5,0xBCC3CAD1,0xD8DFE6ED,0xF4FB0209,
  0x10171E25,0x2C333A41,0x484F565D,0x646B7279
};
static const uint32_t FK[4] = { 0xA3B1BAC6,0x56AA3350,0x677D9197,0xB27022DC };

inline uint32_t rotl(uint32_t x, int n) { return (x << n) | (x >> (32 - n)); }

/* -------------------- T‑tables -------------------- */
array<uint32_t, 256> T0e, T1e, T2e, T3e;   // L  (2,10,18,24)  加/解密
array<uint32_t, 256> T0k, T1k, T2k, T3k;   // L′ (13,23)       密钥扩展

inline uint32_t L_enc(uint32_t b) { return b ^ rotl(b, 2) ^ rotl(b, 10) ^ rotl(b, 18) ^ rotl(b, 24); }
inline uint32_t L_key(uint32_t b) { return b ^ rotl(b, 13) ^ rotl(b, 23); }

void gen_tables() {
    for (int i = 0; i < 256; ++i) {
        uint32_t s = SBOX[i];
        uint32_t le = L_enc(s);
        uint32_t lk = L_key(s);
        T0e[i] = le;            T0k[i] = lk;
        T1e[i] = rotl(le, 8);   T1k[i] = rotl(lk, 8);
        T2e[i] = rotl(le, 16);   T2k[i] = rotl(lk, 16);
        T3e[i] = rotl(le, 24);   T3k[i] = rotl(lk, 24);
    }
}
inline uint32_t Te(uint32_t x) {
    return T0e[(x >> 24) & 0xFF] ^ T1e[(x >> 16) & 0xFF] ^ T2e[(x >> 8) & 0xFF] ^ T3e[x & 0xFF];
}
inline uint32_t Tk(uint32_t x) {
    return T0k[(x >> 24) & 0xFF] ^ T1k[(x >> 16) & 0xFF] ^ T2k[(x >> 8) & 0xFF] ^ T3k[x & 0xFF];
}

/* -------------------- 轮密钥扩展 ------------------ */
array<uint32_t, 32> key_schedule(const uint8_t MK[16]) {
    uint32_t K[36];
    array<uint32_t, 32> rk;
    for (int i = 0; i < 4; ++i) {
        K[i] = (MK[4 * i] << 24) | (MK[4 * i + 1] << 16) | (MK[4 * i + 2] << 8) | MK[4 * i + 3];
        K[i] ^= FK[i];
    }
    for (int i = 0; i < 32; ++i) {
        uint32_t tmp = K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i];
        tmp = Tk(tmp);
        K[i + 4] = K[i] ^ tmp;
        rk[i] = K[i + 4];
    }
    return rk;
}

/* -------------------- 加/解密 --------------------- */
void sm4_encrypt(const uint8_t in[16], uint8_t out[16], const array<uint32_t, 32>& rk) {
    uint32_t X[36];
    for (int i = 0; i < 4; ++i)
        X[i] = (in[4 * i] << 24) | (in[4 * i + 1] << 16) | (in[4 * i + 2] << 8) | in[4 * i + 3];
    for (int i = 0; i < 32; ++i) {
        uint32_t tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i];
        X[i + 4] = X[i] ^ Te(tmp);
    }
    for (int i = 0; i < 4; ++i) {
        uint32_t y = X[35 - i];
        out[4 * i] = y >> 24; out[4 * i + 1] = y >> 16; out[4 * i + 2] = y >> 8; out[4 * i + 3] = y;
    }
}
void sm4_decrypt(const uint8_t in[16], uint8_t out[16], const array<uint32_t, 32>& rk) {
    array<uint32_t, 32> rkr;
    for (int i = 0; i < 32; ++i) rkr[i] = rk[31 - i];
    sm4_encrypt(in, out, rkr);
}

/* -------------------- Demo & Benchmark ----------- */
int main() {
    gen_tables();                          // 生成双 T 表

    uint8_t key[16] = {
        0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,
        0x38,0x39,0x61,0x62,0x63,0x64,0x65,0x66
    };
    uint8_t pt[16] = { 'h','e','l','l','o',',',' ','s','m','4',' ','d','e','m','o','!' };
    uint8_t ct[16], back[16];

    auto rk = key_schedule(key);

    /* --------- 单块加密 / 解密，顺便打印结果 -------- */
    sm4_encrypt(pt, ct, rk);
    sm4_decrypt(ct, back, rk);

    std::cout << "Plain : ";
    for (auto b : pt)  std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)b << ' ';
    std::cout << "\nCipher: ";
    for (auto b : ct)  std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)b << ' ';
    std::cout << "\nDec   : ";
    for (auto b : back) std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)b << ' ';
    std::cout << '\n';

    /* --------- 计时：10000 次，总耗时（毫秒）------- */
    constexpr int N = 10000;

    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < N; ++i) sm4_encrypt(pt, ct, rk);
    auto t1 = std::chrono::high_resolution_clock::now();
    double enc_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
    std::cout << "Encrypt time: " << enc_ms/N << " ms/block\n";

    auto t2 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < N; ++i) sm4_decrypt(ct, back, rk);
    auto t3 = std::chrono::high_resolution_clock::now();
    double dec_ms = std::chrono::duration<double, std::milli>(t3 - t2).count();
    std::cout << "Decrypt time: " << dec_ms/N << " ms/block\n";
}
